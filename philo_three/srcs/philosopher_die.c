...
{
    if (get_current_time() - philosopher->last_time_eating > philosopher->conf->time_to_die)
        return (TRUE);
    return (FALSE);
}
...
{
    if (philosopher->eating_n_times >= philosopher->conf->philosopher_must_eat_times)
        return (TRUE);
    return (FALSE);
}
