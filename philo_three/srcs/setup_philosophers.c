...
static int	setup_semaphores(sem_t **forks_semaphore, sem_t **death_semaphore, t_conf *conf)
{
...
int	setup_philosophers(t_philosopher **philosophers, t_conf *conf, int *exit)
{
    ...
    if (!(*philosophers = (t_philosopher *)malloc(sizeof(t_philosopher) * conf->philosophers_num)))
